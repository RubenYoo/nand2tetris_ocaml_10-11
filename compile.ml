...
(* Adds class variable to symbol table, raising an exception if the symbol is already
    in the table. *)
let add_class_symbol_if_exists
    (symbols : Symbol_table.t)
...
(** Converts symbol info to the string representing the VM memory location
    holding the symbol. *)
let add_params_to_memory_call (info : Symbol_table.symbol_info) : string =
  let memory_segment =
...
let rec compile_expression
  ...
  | Lvalue lval ->
    let (lval_setup, lval_location) = compile_left_val state lval in
...
and compile_left_val
    (state : compiler_state) (lval : Ast_types.lvalue) : string list * string =
  match lval with
  | Variable var_name ->
    let var_info = find_symbol_if_exists state.symbols var_name in
    let variable_location = add_params_to_memory_call var_info in
    ([], variable_location)
  | Array_element (arr_name, index) ->
    let var_info = find_symbol_if_exists state.symbols arr_name in
    let arr_base_location = add_params_to_memory_call var_info in
    (List.concat
       [
    ...
    | Other_call (qualifier, method_name) ->
      ...
      | Some symbol_info ->
        ...
        | Object_type obj_type ->
          (obj_type,
           method_name,
           Some ("push " ^ (add_params_to_memory_call symbol_info)))
  in
...
let rec compile_statement
  ...
  | Let_statement (lval, rval) ->
    ...
    let compiled_r = compile_expression state rval in

    (* Compile the left-hand side lvalue and get its location *)
    let (lval_setup, lval_location) = compile_left_val state lval in

    (* Return the compiled code and updated state *)
    (state,
     List.concat [ compiled_r ; lval_setup ; ["pop " ^ lval_location] ])

  | Do_statement (func_name, params) ->
    (* Generate code to ignore the return value *)
    let ignore_return_value = "pop temp 7" in

    (* Return the compiled subroutine call code and ignore the return value *)
    (state,
  ...
  | Return_statement return_value_opt ->
    ...
    (* Return the compiled code for returning from the subroutine *)
    (state, compiled_return)
  | If_statement (branch_condition, true_block, false_block) ->
    compile_if state branch_condition true_block false_block
...
let compile_subroutine
  ...
  let subroutine_symbols =
    List.fold
      params ~init:state.symbols ~f:(add_symbol ~kind:Symbol_table.Argument)
  in
  let subroutine_symbols =
    List.fold
      subroutine.local_variables
      ~init:subroutine_symbols
      ~f:(add_symbol ~kind:Symbol_table.Local)
  in
...
(** Compiles Jack code to VM code as a list of strings.using descending recursion on AST tree*)
let compile_program (program : Ast_types.class_declaration) : string list =
  let symbol_table = Symbol_table.empty in
  (*fill symbol table with all variables of type "var int blabla"*)
  let symbol_table =
    List.fold program.class_variables ~init:symbol_table ~f:add_class_symbol_if_exists
  in
  let current_state = {
  ...
  (*compile subroutines*)
  List.(program.subroutines >>= compile_subroutine current_state)
